...
export const routeApi = {
  ...
  getRoutePage: (params: RouteQuery) => {
    return request.get<any, PageBody<RouteVO>>('/gateway/route/page', { params })
  },

  // 查询路由列表 - GET /gateway/route/list
  getRouteList: (params?: RouteQuery) => {
    return request.get<any, RouteVO[]>('/gateway/route/list', { params })
  },

  // 根据ID获取路由详情 - GET /gateway/route/{id}
  getRouteById: (id: number | string) => {
    return request.get<any, RouteDTO>(`/gateway/route/${id}`)
  },
  ...
  getInstancesPage: (params: InstanceQuery) => {
    return request.get<any, PageBody<Instance>>('/gateway/route/instances/page', { params })
  },

  // 更新实例权重 - POST /gateway/route/instances/weight
  updateInstanceWeight: (data: InstanceDTO) => {
    return request.post<any, void>('/gateway/route/instances/weight', data)
  },

  // 更新实例启用状态（上下线）- POST /gateway/route/instances/enabled
  updateInstanceEnabled: (data: InstanceDTO) => {
    return request.post<any, void>('/gateway/route/instances/enabled', data)
  },
  ...
  getInterfaces: (id: number | string) => {
    return request.get<any, InterfaceDTO[]>(`/gateway/route/${id}/interfaces`)
  }
}
